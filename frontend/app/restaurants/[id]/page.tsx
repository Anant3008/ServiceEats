...
const MenuCard = ({
  ...
  <div
    className={`group bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex justify-between gap-4 ${!item.isAvailable ? "opacity-60 grayscale" : ""}`}
  >
    {/* Text Content */}
    <div className="flex-1">
      <div className="mb-2">
        ...
        <div
          className={`w-4 h-4 border flex items-center justify-center rounded-[4px] ${item.isVeg !== false ? "border-green-600" : "border-red-600"}`}
        >
          <div
            className={`w-2 h-2 rounded-full ${item.isVeg !== false ? "bg-green-600" : "bg-red-600"}`}
          ></div>
        </div>
      </div>
    ...
    <div className="flex flex-col items-center gap-2 relative">
      ...
      {item.isAvailable && (
        <p className="text-[10px] text-slate-400 mt-3">Customizable</p>
      )}
    </div>
  </div>
);
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <div className="flex items-center gap-4 text-sm font-bold">
                  ...
                  <div className="flex items-center gap-1.5 bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg">
                    ...
                    {restaurant.deliveryTime || "35 min"}
                  </div>
                  <div className="flex items-center gap-1.5 bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg">
                    <span className="text-slate-200">₹</span>{" "}
                    {restaurant.costForTwo || "₹400 for two"}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl max-w-xs">
                ...
                <p className="text-slate-400 text-xs">
                  Use code WELCOME50 | Orders above ₹159
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4 text-sm">
                ...
                <p className="text-slate-500 leading-relaxed text-xs">
                  {restaurant.description ||
                    "Known for hygiene and great taste. Packaging is spill-proof."}
                </p>
              </div>
            </div>
            ...
            {cartCount > 0 && (
              <div className="hidden lg:block bg-slate-900 text-white p-6 rounded-2xl shadow-xl animate-in slide-in-from-bottom-4">
                ...
                <Link
                  href="/cart"
                  className="w-full py-3 bg-orange-600 hover:bg-orange-500 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors"
                >
