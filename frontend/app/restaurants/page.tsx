    ...
    <div className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden h-full flex flex-col">
      ...
      <div className="relative h-56 bg-slate-100 overflow-hidden">
        ...
        <img
          src={
            data.imageUrl ||
            data.image ||
            "https://images.unsplash.com/photo-1504674900247-0877df9cc836?ixlib=rb-4.0.3&w=800&q=80"
          }
          alt={data.name}
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-1">
          ...
          <div
            className={`flex items-center gap-1 px-2 py-0.5 rounded-md border shrink-0 ${getRatingColor()}`}
          >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div
                key={i}
                className="bg-white h-80 rounded-2xl border border-slate-100 p-4 animate-pulse"
              >
