  ...
  return (
    <>
      ...
      <div className={sidebarClasses}>
        ...
        <button
          onClick={onLogout}
          className="flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors font-medium mt-auto text-sm"
        >
...
const AddressCard = ({
  ...
  const Icon =
    (icons as Record<string, typeof MapPin>)[address.label] || MapPin;
  ...
  return (
    <div
      className={`group bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden ${theme.border}`}
    >
      {address.isDefault && (
        <div className="absolute top-0 right-0 bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl shadow-sm z-10">
          DEFAULT
        </div>
      )}
      <div className="flex justify-between items-start mb-4">
        <div
          className={`w-12 h-12 rounded-2xl flex items-center justify-center ${theme.bg} ${theme.text} mb-2`}
        >
          <Icon size={24} />
        </div>
...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      setProfile((prev) =>
        prev ? { ...prev, name: data.user.name, phone: data.user.phone } : null,
      );
  ...
  const handleAddressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      setProfile((prev) =>
        prev ? { ...prev, addresses: data.addresses } : null,
      );
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col lg:flex-row font-sans text-slate-800">
      ...
      <main className="flex-1 p-4 lg:p-10 overflow-y-auto h-screen">
        <div className="max-w-5xl mx-auto pb-20">
          ...
          {status && (
            <div
              className={`fixed bottom-6 right-6 px-6 py-4 rounded-xl shadow-2xl z-50 animate-bounce text-white font-bold ${status.type === "success" ? "bg-green-600" : "bg-red-600"}`}
            >
              {status.msg}
            </div>
          )}
              ...
              <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-orange-600 to-red-600 text-white shadow-2xl shadow-orange-200/50">
                ...
                <div className="relative z-10 p-8 md:p-10 flex flex-col md:flex-row items-center md:items-start gap-8">
                  <div className="relative group">
                    <div className="w-28 h-28 rounded-full bg-white p-1.5 shadow-xl">
                      <img
                        ...
                        alt="Avatar"
                        className="w-full h-full rounded-full bg-orange-50 object-cover"
                      />
                    </div>
                  </div>
                  <div className="text-center md:text-left flex-1">
                    ...
                    <p className="text-orange-100/90 font-medium mb-8">
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-8 border-t border-white/10 pt-6">
                      <div className="text-center md:text-left">
                        ...
                        <p className="text-[10px] text-orange-200 uppercase tracking-widest font-bold mt-1">
                          Orders
                        </p>
                      </div>
                      <div className="w-px h-10 bg-white/10"></div>
                      <div className="text-center md:text-left">
                        <p className="text-3xl font-bold flex gap-1 justify-center md:justify-start">
                          4.8{" "}
                          <Star
                            size={20}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  <button
                    ...
                    className="h-full min-h-[200px] border-2 border-dashed border-slate-200 hover:border-orange-400 bg-slate-50/50 hover:bg-orange-50/50 rounded-2xl flex flex-col items-center justify-center gap-3 text-slate-400 hover:text-orange-600 transition-all group"
                  >
                    <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-all">
                      <Plus size={24} />
                    </div>
      ...
      >
        <form onSubmit={handleUpdateProfile} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-3 bg-orange-600 text-white font-bold rounded-xl hover:bg-orange-700 transition flex justify-center gap-2 shadow-lg shadow-orange-200"
          >
      ...
      <Modal
        ...
        title={isEditingAddress ? "Edit Address" : "New Address"}
      >
        <form onSubmit={handleAddressSubmit} className="space-y-4">
          ...
          <div
            className="flex items-center gap-2 mt-2 p-3 bg-slate-50 rounded-xl cursor-pointer"
            onClick={() =>
              setAddressForm({
                ...addressForm,
          ...
          >
            <div
              className={`w-5 h-5 rounded border flex items-center justify-center transition ${addressForm.isDefault ? "bg-orange-600 border-orange-600" : "bg-white border-slate-300"}`}
            >
