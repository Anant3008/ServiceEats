...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    ...
    const getNextDelay = (
      ...
      if (
        nextDelivery?.status === "on_the_way" ||
        nextDelivery?.status === "picked_up"
      ) {
        return 5000;
      }
  ...
  const getProgressStep = () => {
    ...
    if (delivery?.status === "on_the_way" || delivery?.status === "picked_up")
      return 3; // On Way
  ...
  const ProgressBar = () => {
    ...
    const progressPercent = Math.max(
      0,
      Math.min(100, ((currentStep - 1) / 3) * 100),
    );
                ...
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center border-4 transition-all duration-500 z-10 bg-white ${
                    isCompleted
                      ...
                      : isCurrent
                        ? "border-orange-500 text-orange-600 shadow-[0_0_0_4px_rgba(249,115,22,0.2)] scale-110"
    ...
    switch (currentStep) {
      case 1:
        return {
          ...
          subtitle: "The restaurant is working on your order.",
          color: "bg-orange-500",
        };
      case 2:
        return {
          title: "Driver Assigned",
          subtitle: `${delivery?.driverName || "A driver"} is heading to the restaurant.`,
          color: "bg-blue-600",
        };
      case 3:
        return {
          title: "Order on the way",
          subtitle: "Your food is out for delivery!",
          color: "bg-purple-600",
        };
      case 4:
        return {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 pb-20">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div
          className={`${hero.color} text-white rounded-[2.5rem] p-8 md:p-12 mb-10 shadow-xl transition-colors duration-500 relative overflow-hidden`}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl">
                    ...
                    <div className="flex justify-center gap-2 mb-4">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <button
                          key={s}
                          onClick={() => setRating({ ...rating, order: s })}
          ...
          <div className="space-y-6">
            ...
            <div className="text-center">
              <button
                onClick={() => router.push("/restaurants")}
                className="text-xs font-bold text-slate-400 hover:text-orange-600 transition uppercase tracking-widest"
              >
                Report an Issue
              </button>
            </div>
