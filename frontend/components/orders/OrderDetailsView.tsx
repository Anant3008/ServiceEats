...
export default function OrderDetailsView({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Delivery
        try {
          const dRes = await fetch(`${API_BASE}/api/deliveries/${orderId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
        ...
        try {
          const rateRes = await fetch(
            `${API_BASE}/api/ratings/order/${orderId}`,
            { headers: { Authorization: `Bearer ${token}` } },
          );
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      setRatingError(
        getErrorMessage(err, "Failed to submit rating. Please try again."),
      );
      console.error(err);
    } finally {
    ...
    return (
      <div className="w-full py-6">
        <div className="flex items-center justify-between relative">
          ...
          <div
            className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-green-500 -z-10 transition-all duration-1000"
          ...
          {steps.map((label, idx) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center border-4 text-xs font-bold ${idx <= step ? "bg-green-500 border-green-500 text-white" : "bg-slate-100 text-slate-300"}`}
              >
                {idx + 1}
              </div>
  ...
  return (
    <div className="animate-in slide-in-from-right-4 duration-300">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-slate-500 hover:text-orange-600 font-bold mb-6"
      >
              ...
              {order.items.map((item, idx) => (
                <div key={idx} className="py-3 flex justify-between">
                  <div className="flex gap-3">
                    <div className="w-6 h-6 bg-orange-100 text-orange-700 rounded flex items-center justify-center text-xs font-bold">
                      {item.quantity}x
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        value={reviews.order}
                        onChange={(e) =>
                          setReviews({ ...reviews, order: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <div>
                      <StarInput
                        ...
                        onChange={(v) =>
                          setRatings({ ...ratings, restaurant: v })
                        }
                      />
                      <textarea
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-orange-200 resize-none"
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-900 text-white p-6 rounded-2xl relative overflow-hidden">
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold">{delivery.driverName}</p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100">
            ...
            <div className="space-y-2 text-sm text-slate-600 mb-4">
              ...
              <div className="flex justify-between">
                <span>Tax (5%)</span>
                <span>₹{(order.totalAmount * 0.05).toFixed(0)}</span>
              </div>
              <div className="flex justify-between">
                <span>Delivery</span>
                <span>₹50</span>
            ...
            <div className="border-t pt-3 flex justify-between font-black text-slate-900">
              <span>Total</span>
              <span>₹{(order.totalAmount * 1.05 + 50).toFixed(0)}</span>
            </div>
          </div>
        </div>
      </div>
