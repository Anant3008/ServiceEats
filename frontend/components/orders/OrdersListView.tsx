...
export default function OrdersListView({
    ...
    const fetchOrders = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/orders/user/${userId}?page=${page}&limit=10`,
          {
  ...
  const getBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-lg text-xs font-bold capitalize ${styles[status] || "bg-gray-50"}`}
      >
        {status}
      </span>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      {orders.map((order) => (
        <div
          key={order._id}
          className="bg-white p-6 rounded-2xl border border-slate-100 hover:shadow-md transition group"
        >
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-2 bg-slate-50 text-slate-700 font-bold rounded-xl text-xs hover:bg-slate-100 transition flex items-center gap-1"
                >
