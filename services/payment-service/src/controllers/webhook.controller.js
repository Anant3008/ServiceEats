...
async function handlePaymentSucceeded(paymentIntent) {
  ...
  try {
    ...
    console.log(
      `Kafka event 'payment.succeeded' emitted for order ${payment.orderId}`,
    );
  } catch (err) {
    console.error(
      `Error handling payment succeeded for ${paymentIntent.id}:`,
      err,
    );
...
async function handlePaymentFailed(paymentIntent) {
  ...
  try {
    ...
    console.log(
      `Kafka event 'payment.failed' emitted for order ${payment.orderId}`,
    );
  } catch (err) {
    console.error(
      `Error handling payment failed for ${paymentIntent.id}:`,
      err,
    );
    throw err;
  }
...
async function handlePaymentCancelled(paymentIntent) {
  ...
  try {
    ...
    console.error(
      `Error handling payment cancelled for ${paymentIntent.id}:`,
      err,
    );
    throw err;
  }
